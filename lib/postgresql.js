...
PostgreSQL.prototype.fromColumnValue = function(prop, val) {
  ...
  var nativeType = prop.postgresql && prop.postgresql.dataType;
  ...
  } else if (type === 'String' && nativeType === 'date') {
...
PostgreSQL.prototype.toColumnValue = function(prop, val) {
  ...
  if (prop.type === Date || prop.type.name === 'Timestamp') {
    ...
    const dataType = prop.postgresql && prop.postgresql.dataType
    ...
    switch (dataType) {
      ...
      default:
        castTemplate = '?::TIMESTAMP WITH TIME ZONE';
    }
